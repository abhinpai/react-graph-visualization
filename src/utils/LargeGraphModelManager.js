...
import { webGlSupported } from "./Workarounds.js";

export class LargeGraphModelManager extends GraphModelManager {
  constructor(canvas, contentGroup) {
    ...
    this.$graphComponent.addZoomChangedListener(
      this.onGraphComponentZoomChanged.bind(this)
    );
    this.$graphComponent.addGraphChangedListener(
      this.onGraphComponentGraphChanged.bind(this)
    );
  ...
  updateEffectiveStyle() {
    ...
    if (manager.graphOptimizationMode === OptimizationMode.LEVEL_OF_DETAIL) {
      ...
      if (manager.graphComponent.zoom < manager.overviewZoomThreshold) {
        ...
        manager.graphComponent.zoom < manager.intermediateZoomThreshold
      ) {
